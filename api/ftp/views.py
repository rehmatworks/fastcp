...
class FTPUserViewSet(ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        subprocess.run(
            ...
            input=(
                f'{serializer.validated_data["password"]}\n'
                f'{serializer.validated_data["password"]}\n'
            ).encode(),
            check=True,
        )
        ...
        subprocess.run(
            [
                'docker', 'exec', 'fastcp-ftp', 'pure-pw', 'mkdb',
                '/etc/pure-ftpd/pureftpd.pdb', '-f',
                '/etc/pure-ftpd/passwd/pureftpd.passwd',
            ],
            check=True,
        )
    ...
    def perform_destroy(self, instance):
        ...
        subprocess.run(
            [
                'docker', 'exec', 'fastcp-ftp',
                'pure-pw', 'userdel', instance.username,
                '-f', '/etc/pure-ftpd/passwd/pureftpd.passwd',
                '-m',
            ],
            check=True,
    ...
    def reset_password(self, request, pk=None):
        ...
        subprocess.run(
            [
                'docker', 'exec', 'fastcp-ftp',
                'pure-pw', 'passwd', instance.username,
                '-f', '/etc/pure-ftpd/passwd/pureftpd.passwd',
                '-m',
            ],
