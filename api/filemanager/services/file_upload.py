...
class FileUploadService(BaseService):
    ...
    def upload_file(self, validated_data: dict) -> bool:
        ...
        Args:
            validated_data (dict): Validated data dict from serializer
                (api.filemanager.serializers.FileUploadSerializer)
        ...
        try:
            if self.is_allowed(path, user) and not os.path.exists(dest_path):
                ...
                logging.warning(
                    f"Upload not allowed or file exists: {dest_path}"
                )
        except Exception:
            logging.error(f"Error uploading file to {dest_path}")
        return False
    ...
    def remote_upload(self, validated_data: dict) -> bool:
        ...
        Args:
            validated_data (dict): Validated data dict from serializer
                (api.filemanager.serializers.RemoteUploadSerializer)
        ...
        try:
            if self.is_allowed(path, user):
                with open_url(remote_url, stream=True) as res:
                    if res.status_code == 200:
                        ...
                        logging.warning(
                            f"Remote upload failed, status {res.status_code}: "
                            f"{remote_url}"
                        )
            else:
                logging.warning(f"Remote upload not allowed: {dest_path}")
        except Exception:
            logging.error(f"Error in remote upload to {dest_path}")
        return False
